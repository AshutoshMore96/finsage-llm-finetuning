...
def main() -> None:
    ...
    # Resume-safe: load prompts already saved so a Colab disconnect doesn't lose
    # work — re-running continues where it left off instead of starting over.
        ...
        with open(out_path) as f:
            for line in f:
                try:
                    ...
                    pass
    if done_prompts:
        print(f"· Resuming: {len(done_prompts)} pairs already on disk — skipping those.")

    model, tokenizer = load_base_model(cfg)

    kept = len(done_prompts)
    # Append mode + flush => every pair is durable the instant it's generated.
    with open(out_path, "a") as fout:
        for i, r in enumerate(rows):
            ...
            fout.write(json.dumps(
                {"prompt": prompt_text, "chosen": gold, "rejected": rejected}) + "\n")
            fout.flush()  # persist immediately
            kept += 1
            if (i + 1) % 50 == 0:
                print(f"  generated {i + 1}/{len(rows)} (kept {kept})")

    print(f"\n✅ Wrote {kept} DPO pairs -> {out_path}")
